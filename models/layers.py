...
class ResBlock(nn.Module):
    def __init__(self, in_channel, out_channel, filter=False):
        ...
        self.poolLayer = DeepPoolLayer(in_channel, out_channel) if filter else nn.Identity()
        self.conv2 = BasicConv(out_channel, out_channel, kernel_size=3, stride=1, relu=False)

    def forward(self, x, kernel):
        x = self.conv1(x)
        if self.filter:
            x = self.poolLayer(x, kernel) 
        else:
            x = self.poolLayer(x)
        x = self.conv2(x)
        return x
...
class DeepPoolLayer(nn.Module):
    ...
    def forward(self, x, kernel):
        ...
        for i in range(len(self.pools_sizes)):
            if i == 0:
                y = self.dynas[i](self.convs[i](self.pools[i](x)), kernel)
            else:
                y = self.dynas[i](self.convs[i](self.pools[i](x)+y_up), kernel)
...
class dynamic_filter(nn.Module):
    ...
    def forward(self, x, kernel):
        ...
        low_filter = low_filter.reshape(n, c1//self.kernel_size**2, self.kernel_size**2, p*q).unsqueeze(2)+0.1*kernel #这个地方就是生成出来的卷积核 shape为(n, group, 1, k*k, h*w)
