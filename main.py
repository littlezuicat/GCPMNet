    ...
    with tqdm(range(start_step + 1, opt.steps + 1), unit='step', ncols=130) as steps:
        with open(log_file, 'a') as f:
            for step in steps:  
                ...
                x, kernel, y = next(iter(loader_train))
                x = x.to(opt.device)
                y = y.to(opt.device)
                kernel = kernel.to(opt.device)
                
                outputs = net(x, kernel)
...
def test(net, loader_test, max_psnr, max_ssim, step):
    ...
    for i, (inputs, kernel, targets) in enumerate(loader_test):
            inputs = inputs.to(opt.device)
            targets = targets.to(opt.device)
            kernel = kernel.to(opt.device)
            output_list = net(inputs, kernel)

            ssim1 = ssim(output_list[-1].clamp(0,1), targets).item()
            psnr1 = psnr(output_list[-1].clamp(0,1), targets)

            ssims.append(ssim1)
            psnrs.append(psnr1)
            outputs.append(output_list[-1])
            # Save images
            ts = torch.squeeze(output_list[-1].clamp(0, 1).cpu())
